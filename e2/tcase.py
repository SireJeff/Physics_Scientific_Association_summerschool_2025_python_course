import os

# Define the test cases for Question 2: (input_content, output_content)
# Input format: "mass velocity"
test_cases = [
    ("10 5", "125.0"),
    ("2 100", "10000.0"),
    ("1 1", "0.5"),
    ("1000 1000", "500000000.0"),
    ("50 20", "10000.0"),
    ("999 1", "499.5"),
]

# Create the 'in' and 'out' directories
os.makedirs("in", exist_ok=True)
os.makedirs("out", exist_ok=True)

# Loop through the test cases and create the files
for i, (input_data, output_data) in enumerate(test_cases, 1):
    # Create input file
    with open(os.path.join("in", f"input{i}.txt"), "w") as f:
        f.write(input_data)

    # Create output file
    with open(os.path.join("out", f"output{i}.txt"), "w") as f:
        f.write(output_data)

print("Test case folders for Question 2 created successfully!")